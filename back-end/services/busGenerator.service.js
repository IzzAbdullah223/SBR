...
export const generateDirectBuses = (
  ...
  const allArrivalTimes = scheduleGen.generateArrivalTimes(
    frequency,
    dubaiTime,
    15
  );
  ...
  let arrivalTimes = allArrivalTimes
    .filter(
      (dep) => !isNaN(dep.minutesFromNow) && dep.minutesFromNow >= minDeparture
    )
    .slice(0, 5);
  ...
  arrivalTimes.forEach((arrival, i) => {
    ...
    const busDepatureDate = timeHelper.addMinutes(
      dubaiTime,
      arrival.minutesFromNow
    );
    const destinationArrival = timeHelper.addMinutes(
      busDepatureDate,
      routeTravelTime
    );
    ...
    buses.push({
      ...
      // ✅ NEW — actual clock time user arrives at destination
      destinationArrivalTime: timeHelper.formatTime(destinationArrival),
      destinationArrivalTime24: timeHelper.formatTime24(destinationArrival),
      totalJourneyTime,
...
export const generateTransferBuses = async (
  ...
  const allLeg1Arrivals = scheduleGen.generateArrivalTimes(
    frequency1,
    dubaiTime,
    15
  );
  ...
  const transferIndex = route1Stops.findIndex(
  ...
  const leg1RidingTime = leg1Duration * transferRatio;
  const leg2RidingTime = leg2Duration * (1 - transferRatio);

  const totalTime = leg1RidingTime + transferWaitTime + leg2RidingTime; // total time from departure till destination
  ...
  leg1Arrivals.forEach((leg1Arrival, i) => {
    ...
    const leg2DepartureTime = timeHelper.addMinutes(
      dubaiTime,
      leg2DepartureMin
    );
...
export const generateAllBuses = async (
  ...
  if (routeData.transferRoutes.length > 0) {
    const transferStopIds = routeData.transferRoutes.map(
      (t) => t.transferStopId
    );
    const transferStops = await BusStop.find({
      stopId: { $in: transferStopIds },
    });

    const transferStopMap = {};
    transferStops.forEach((s) => {
      transferStopMap[s.stopId] = s;
    });
  ...
  for (const routeInfo of routeData.directRoutes) {
    const buses = generateDirectBuses(
      routeInfo,
      origin,
      destination,
      dubaiTime
